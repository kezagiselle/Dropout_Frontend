        ...
        <main className="flex-1 min-w-0 p-3 sm:p-4 lg:p-6">
          ...
          <div className="bg-white border-b border-gray-200 px-4 py-4 sm:px-6 lg:px-8 mb-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-gray-500 text-sm sm:text-base">View your daily and weekly class schedule</p>
              </div>
              <div className="flex flex-wrap items-center gap-2 text-sm">
                <div className="relative">
                  <select className="appearance-none bg-white border border-gray-200 rounded-lg pl-4 pr-8 py-2 font-medium text-gray-700 shadow-sm">
                ...
                <button className="flex items-center gap-2 px-4 py-2 rounded-lg border border-blue-200 text-blue-600 bg-blue-50 font-semibold shadow-sm">
                  <Clock className="w-4 h-4" />
                  Week View
                </button>
                <button className="flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-200 text-gray-700 bg-white shadow-sm">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-4 lg:col-span-1">
                ...
                <div className="space-y-3">
                  {nextClasses.map((cls, idx) => (
                    ...
                    >
                      ...
                      <h3 className="text-base font-semibold text-gray-900 mt-1">{cls.title}</h3>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                <p className="text-xs uppercase tracking-widest text-gray-400 mb-3">Alerts</p>
                <div className="space-y-3">
                  {alerts.map((alert, idx) => (
                    <div key={idx} className={`p-3 rounded-xl border border-gray-100 ${alert.color.replace('text-', 'border-')}`}>
            ...
            {/* Main Timetable */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 lg:col-span-3">
              <div className="flex flex-wrap items-center justify-between mb-4 gap-3">
                <div>
                  <p className="text-sm font-semibold text-gray-900">November 18-22, 2024</p>
                  <p className="text-xs text-gray-500">Week 12 • Term 2</p>
                </div>
                <div className="flex items-center gap-2">
                  <button className="px-4 py-2 text-sm font-semibold text-emerald-600 bg-emerald-50 rounded-lg flex items-center gap-2">
                    <MessageSquare className="w-4 h-4" />
                    Message Teacher
                  </button>
                  <button className="px-4 py-2 text-sm font-semibold text-gray-600 border border-gray-200 rounded-lg">
                    Today
                  </button>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  <thead>
                    <tr>
                      <th className="w-24 text-left text-xs font-semibold uppercase tracking-wider text-gray-400 pb-3">Time</th>
                      {schedule.map((day) => (
                        <th key={day.day} className="text-left text-sm font-semibold text-gray-600 pb-3">
                          {day.day}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {Array.from({ length: 4 }).map((_, rowIdx) => (
                      <tr key={rowIdx}>
                        <td className="align-top pr-4 pb-4 text-xs font-semibold text-gray-400">
                        ...
                        {schedule.map((day) => {
                          ...
                          return (
                            <td key={`${day.day}-${rowIdx}`} className="p-2 pb-4 align-top">
                              {slot ? (
                                <div className={`p-3 rounded-xl border h-full ${baseClass}`}>
                                  ...
                                  {slot.badge && (
                                    <span className={`inline-flex items-center mt-2 px-2 py-1 text-xs font-semibold rounded-full ${slot.badgeColor}`}>
                                      {slot.badge}
                                    </span>
                                  )}
                                </div>
                              ) : (
                                <div className="h-full rounded-xl border border-dashed border-gray-200"></div>
                              )}
                            </td>
                          );
                        })}
